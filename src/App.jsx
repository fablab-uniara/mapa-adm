    ...
    <div style={{ minHeight: "100vh", background: "#f8f9fb", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans','Helvetica Neue',sans-serif" }}>
      ...
      <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e5e7eb", padding: "48px 40px", width: 380, textAlign: "center", boxShadow: "0 4px 24px rgba(0,0,0,0.06)" }}>
        ...
        <button onClick={onLogin} disabled={loading}
          style={{ width: "100%", padding: "12px 0", borderRadius: 9, border: "1px solid #e5e7eb", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, fontSize: 14, fontWeight: 600, color: "#374151", cursor: loading ? "wait" : "pointer", transition: "all 0.15s ease", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}
  ...
  return (
    <svg width={size} height={size} style={{ overflow: "visible" }}>
      ...
      {data.map((d, i) => {
        ...
        return <g key={i}><circle cx={dx} cy={dy} r="3.5" fill={color} /><text x={lx} y={ly} textAnchor="middle" dominantBaseline="middle" style={{ fontSize: 9, fill: "#6b7280", fontFamily: "'DM Sans',sans-serif", fontWeight: 500 }}>{d.name}</text></g>;
      })}
    </svg>
  );
}
  ...
  return (
    <div className="disc-card" onClick={() => setSelectedDisc(disc.id)}
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 7 }}>
        <div style={{ padding: "1px 6px", background: done ? "rgba(255,255,255,0.15)" : `${color}12`, borderRadius: 3, fontSize: 9, fontWeight: 600, color: done ? "rgba(255,255,255,0.7)" : color, letterSpacing: "0.3px", maxWidth: "82%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{disc.area.toUpperCase()}</div>
          ...
          {nextUnlocked.length === 0 ? <p style={{ fontSize: 12, color: "#9ca3af", fontStyle: "italic" }}>{missingDiscs.length === 0 ? "🎉 Trilha completa!" : "Complete os pré-requisitos primeiro."}</p> : (
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              {nextUnlocked.map(disc => (
                <div key={disc.id} onClick={() => setSelectedDisc(disc.id)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 9px", borderRadius: 7, background: "#f9fafb", cursor: "pointer", border: "1px solid transparent" }} onMouseEnter={e => e.currentTarget.style.border = `1px solid ${career.color}44`} onMouseLeave={e => e.currentTarget.style.border = "1px solid transparent"}>
                  ...
                  <div className="check-anim" onClick={e => { e.stopPropagation(); toggleCompleted(disc.id); }} style={{ width: 18, height: 18, borderRadius: 4, border: `1.5px solid ${career.color}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0, color: career.color, fontSize: 12, fontWeight: 700 }}>+</div>
                </div>
              ))}
            </div>
          )}
...
export default function App() {
  ...
  const saveProgress = async (newSet) => {
    ...
    setSaving(true);
    try {
      await setDoc(doc(db, "progress", user.uid), { completed: [...newSet], updatedAt: new Date().toISOString() });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleLogout = () => { signOut(auth); setCompleted(new Set()); };
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 62, position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4, background: "#f3f4f6", borderRadius: 9, padding: 3 }}>
          {[["mapa", "🗺️ Mapa"], ["gap", "📊 Gap Analysis"]].map(([v, l]) => (
            <button key={v} className="nav-btn" onClick={() => setMainView(v)} style={{ padding: "6px 16px", borderRadius: 7, background: mainView === v ? "#fff" : "transparent", boxShadow: mainView === v ? "0 1px 4px rgba(0,0,0,0.10)" : "none", fontSize: 12, fontWeight: mainView === v ? 600 : 400, color: mainView === v ? "#1a1a2e" : "#6b7280" }}>{l}</button>
          ))}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
              <div style={{ width: 90, height: 5, background: "#e5e7eb", borderRadius: 3, overflow: "hidden" }}><div className="prog-bar" style={{ width: `${totalProgress}%`, height: "100%", background: "#1d4ed8", borderRadius: 3 }} /></div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, borderLeft: "1px solid #e5e7eb", paddingLeft: 12 }}>
            ...
            <div><div style={{ fontSize: 11, fontWeight: 600, color: "#374151" }}>{user.displayName?.split(" ")[0]}</div><div style={{ fontSize: 9, color: "#9ca3af" }}>{user.email}</div></div>
        ...
        <div style={{ display: "flex", height: "calc(100vh - 62px)" }}>
          ...
          {selectedDiscInfo && (
            <div style={{ width: 268, background: "#fff", borderLeft: "1px solid #e5e7eb", padding: 18, overflowY: "auto", flexShrink: 0 }} className="fade-in">
              ...
              <button onClick={() => { if (isUnlocked(selectedDiscInfo) || completed.has(selectedDiscInfo.id)) toggleCompleted(selectedDiscInfo.id); }} disabled={!isUnlocked(selectedDiscInfo) && !completed.has(selectedDiscInfo.id)}
